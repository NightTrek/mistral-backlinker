...
def generate_lead_email_tasks(customer = jamie):
    ...
    return [
        ...
        Task(
            ...
            context=[customer_research, backlinker_sales_advice],
            agent=Email_writter,
            output_file=f"./email_leads_output/email_{customer.business_name}.txt"
        )
    ]

def sequential_lead_processing(customers):
...
def multi_threaded_crew_runner(customers = [jamie], num_threads = 4):
